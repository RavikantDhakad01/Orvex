...
const getUserInvitations = async (req, res, next) => {
    try {
        const userPendingInvitations = await Invitation.find({ receiver: req.user._id, status: INVITATION_STATUS.PENDING }).populate("workspace", "name  avatarColor").populate("sender", "username email")
